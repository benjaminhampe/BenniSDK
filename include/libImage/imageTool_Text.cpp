...
{
	dbPRINT( "video::Tool::drawText()\n" );

	if (!dst)
	{
		dbERROR( "video::Tool::drawText() - Invalid pointer to IImage\n" );
		return false;
	}

	if (!env)
	{
		dbERROR( "video::Tool::drawText() - Invalid pointer to IGUIEnvironment\n" );
		return false;
	}
	...
	{
		dbERROR( "video::Tool::drawText() - Cant draw outside destination Image ( x coord too big )\n");
		return false;
	}

	if (y>=(s32)dst_size.Height)
	{
		dbERROR( "video::Tool::drawText() - Cant draw outside destination Image ( y coord too big )\n");
		return false;
	}
	...
	if (!img)
	{
		dbERROR( "video::Tool::drawText() - Could not create image\n" );
		return false;
	}
	...
	if (!driver)
	{
		dbERROR( "video::Tool::drawText() - Invalid pointer to IVideoDriver\n" );
		return false;
	}
