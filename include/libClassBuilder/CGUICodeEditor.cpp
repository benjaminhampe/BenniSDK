			...
			for (s32 i=0; i < lineCount; ++i)
			{
				...
				if (!SearchingForEndOfGroup) {
					for (int u = 0; u < lineCStr.size(); u++) {
						...
						for (int v = 0; v < KeywordList.size(); v++) {
							...
							// Check group keywords
								...
								if (u+GroupKeywords[index2].text.size() <= lineCStr.size()) {
									bool match = true;
									int begIndex = 0;
									for (int w = 0; w < GroupKeywords[index2].text.size(); w++) {
										begIndex = u+w;
									...
									if (match) {
										...
										for (int x = 0; x < begIndex-(GroupKeywords[index2].text.size()-1); x++) {
										...
										for (int w = begIndex+1; w < lineCStr.size(); w++) {
											// Look for the end
											bool matchEnd = true;
											int endIndex = u+1;
											for (int x = 0; x < GroupKeywords[index2].endText.size(); x++) {
												endIndex = w+x;
							...
							// Only check if there is enough space left for there to be the keyword
								if (u+LineKeywords[index3].text.size() < lineCStr.size()) {
									bool match = true;
									for (int w = 0; w < LineKeywords[index3].text.size(); w++) {
				...
				if (SearchingForEndOfGroup) {
					...
					for (int u = 0; u < lineCStr.size(); u++) {
						...
						if (u+EndOfGroupText.size() <= lineCStr.size()) {
							for (int w = 0; w < EndOfGroupText.size(); w++) {
							...
							if (match) {
								...
								irr::core::stringw prevText = L"";
								irr::core::stringw thisText = L"";
								for (int x = 0; x < u+EndOfGroupText.size(); x++) {
								    thisText += txtLine->c_str()[x];
								}
								overlay.text = thisText;
								int startX = 0;
				...
				if (startFound && !endFound) {
					...
					overlay.color = GroupColor;
					irr::core::stringw thisText = L"";
					for (int x = startIndex; x < lineCStr.size(); x++) {
						thisText += txtLine->c_str()[x];
					}
					overlay.text = thisText;
					int width = font->getDimension(overlay.text.c_str()).Width;
				...
				// Draw keywords
				for (int u = 0; u < keywordOverlays.size(); u++) {
				...
				if ((focus || InMenu || Environment->getFocus() == Scrollbar) && MarkBegin != MarkEnd && i >= hlineStart && i < hlineStart + hlineCount)
				{
					...
					// draw mark
					irr::video::SColor selColor(255,70,70,180);
					if (InMenu)
						selColor = irr::video::SColor(255,130,130,180);
	...
	case irr::EMIE_RMOUSE_LEFT_UP:
		{
			...
			irr::gui::IGUIContextMenu* menu = Environment->addContextMenu(rect,this);
			OldMarkBegin = MarkBegin;
			OldMarkEnd = MarkEnd;
			menu->addItem(L"Cut");
			menu->addItem(L"Copy");
	...
	case irr::EMIE_LMOUSE_DOUBLE_CLICK:
		{
			...
			for (int i = CursorPos; i < Text.size(); i++) {
				char character;
				wcstombs(&character,&Text.c_str()[i],1);
				if (!isalnum(character)) {
...
void CGUIEditBoxIRB::inputChar(wchar_t c)
{
	...
	if (c != 0)
	{
		if (Text.size() < Max || Max == 0)
		{
			...
			{
				// add new character
				if (Inserting && CursorPos+1 <= Text.size()-1) 
...
void CGUIEditBoxIRB::addGroupKeyword(irr::core::stringc keyword, irr::core::stringc endKeyword, irr::video::SColor color, bool matchCase, bool multiLine) {
	...
	for (int i = 0; i < GroupKeywords.size(); i++) {
